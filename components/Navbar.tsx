...
export default function Navbar() {
  ...
  const mobileIconPillStyle =
    "h-14 w-14 bg-white/10 backdrop-blur-xl border border-white/20 shadow-lg rounded-full flex items-center justify-center transition-transform active:scale-95";
  ...
  // Filter links for the Mobile Hamburger Menu (The rest)
  ...
  return (
    <>
      ...
      >
        ...
        {/* Desktop Links */}
        <div className="flex gap-9 text-white text-[17px] font-medium items-center mr-8">
          {navLinks.map((link) => (
            <Link 
              key={link.name} 
              href={link.href}
              className={`hover:text-cyan-400 transition-colors ${link.name === 'IAM' ? 'font-bold text-cyan-300' : ''}`}
            >
              {link.name}
            </Link>
          ))}
      ...
      {/* 3. Hamburger Dropdown (REDUCED items) */}
      {isMenuOpen && (
        <div
          className="
            md:hidden
          ...
          <div className="flex flex-col items-center gap-6 text-white text-lg font-medium">
            {mobileDropdownLinks.map((link) => (
              <Link 
                ...
                href={link.href} 
                onClick={closeMenu}
                className={link.name === 'IAM' ? 'text-cyan-300 font-bold' : ''}
              >
                {link.name}
              </Link>
            ))}
      ...
      {/* 4. NEW: Bottom Tab Bar (Home, Events, IAM) */}
      <div className="
        md:hidden
...
function HomeIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
      <path d="M11.47 3.84a.75.75 0 011.06 0l8.635 8.635a.75.75 0 01-1.06 1.06l-.315-.315V20.25a.75.75 0 01-.75.75H14.25a.75.75 0 01-.75-.75V14.25H10.5v6a.75.75 0 01-.75.75H5.25a.75.75 0 01-.75-.75V13.22l-.315.315a.75.75 0 01-1.06-1.06l8.635-8.635z" />
    </svg>
  );
}
