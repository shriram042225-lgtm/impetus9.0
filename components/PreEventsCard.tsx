...
import { Calendar, ArrowUpRight, Users, ChevronRight, ChevronLeft } from "lucide-react";
    ...
    <div className="flex flex-col h-full w-full">
      ...
      <div className="p-6 flex flex-col h-[45%]">
        ...
        <div className="mt-auto flex items-center justify-between pt-2 border-t border-white/5">
           ...
           <button 
             onClick={(e) => {
                // Stop propagation so clicking button doesn't trigger a drag
               e.stopPropagation();
               onRegister();
             }}
             // Added z-20 to ensure button is clickable over drag area
             className="relative z-20 bg-white text-black p-3 rounded-full hover:bg-yellow-500 transition-colors active:scale-95"
           >
...
function DesktopEventCard({ event, onRegister }: { event: any; onRegister: () => void }) {
    ...
    const teamSizeLabel = event.teamSize.min === event.teamSize.max 
      ? event.teamSize.max 
      ...
      <div className="group relative flex flex-col bg-zinc-900 border border-white/10 rounded-3xl overflow-hidden hover:border-yellow-500/50 transition-all duration-500 h-full">
        ...
        <div className="relative h-64 w-full overflow-hidden">
          ...
          <div className="absolute top-4 left-4 flex flex-col gap-2">
             <div className="bg-black/60 backdrop-blur-md px-3 py-1 rounded-full border border-white/10 flex items-center gap-2 text-xs font-mono text-zinc-300">
               <Calendar size={12} className="text-yellow-500"/> {event.date}
             </div>
          </div>
        </div>
        ...
        <div className="flex flex-col flex-grow mt-2 p-6 md:p-8">
          ...
          <div className="flex items-center justify-between mt-auto pt-6 border-t border-white/5">
            <div className="flex items-center gap-2 text-xs font-mono text-zinc-500 border border-zinc-800 px-3 py-1.5 rounded bg-zinc-900/50">
               <Users size={12} />
               <span>Team: {teamSizeLabel}</span>
            </div>
  
            {isClosed ? (
              <button
                disabled
                className="px-5 py-2 rounded-full bg-zinc-800 text-zinc-500 text-xs font-bold uppercase tracking-wider cursor-not-allowed border border-zinc-700"
              >
                Closed
              </button>
            ) : (
              <button
                onClick={onRegister}
                className="group/btn flex items-center gap-2 bg-white text-black px-5 py-2 rounded-full text-xs font-bold uppercase tracking-wider hover:bg-yellow-400 transition-colors"
              >
                Register 
